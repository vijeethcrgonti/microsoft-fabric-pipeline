...
def build_fct_orders(run_date: str) -> DataFrame:
    ...
    orders = (
        spark.read.format("delta").load(f"{SILVER_BASE}/silver_orders")
    ...
    return (
        orders
        .join(products.select("product_id", "category", "margin_pct"), on="product_id", how="left")
        ...
        .select(
            ...
            F.col("unit_price"),
            F.col("order_amount"),
            F.round(F.col("order_amount") * F.col("margin_pct") / 100, 2).alias("estimated_margin"),
            F.col("order_date"),
            F.col("order_year"),
            F.col("order_month"),
            F.col("status"),
...
def build_fct_revenue(run_date: str) -> DataFrame:
    ...
    return (
        ...
        .withColumn("margin_pct", F.round(F.col("total_margin") / F.col("total_revenue") * 100, 2))
...
if __name__ == "__main__":
    ...
    merge_delta(spark, fct_ltv, f"{GOLD_BASE}/gold_fct_customer_ltv", "customer_id_hash")
