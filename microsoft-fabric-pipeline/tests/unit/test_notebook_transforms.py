...
def bronze_orders(spark):
    ...
    data = [
        ("ORD-001", "CUST-A", "STR-1", "PRD-1", 2, 50.00, 100.00, date(2024, 1, 15), "completed", datetime(2024, 1, 15, 10, 0)),
        ("ORD-001", "CUST-A", "STR-1", "PRD-1", 2, 50.00, 100.00, date(2024, 1, 15), "completed", datetime(2024, 1, 15, 10, 5)),  # duplicate
        ("ORD-002", None,     "STR-2", "PRD-2", 1, 200.00, 200.00, date(2024, 1, 15), "pending", datetime(2024, 1, 15, 11, 0)),   # null customer
        ("ORD-003", "CUST-B", "STR-1", "PRD-3", 3, 10.00, -30.00, date(2024, 1, 15), "refunded", datetime(2024, 1, 15, 12, 0)),  # negative amount
        ("ORD-004", "CUST-C", "STR-3", "PRD-1", 1, 75.00, 75.00,  date(2024, 1, 15), "  pending  ", datetime(2024, 1, 15, 13, 0)),
    ]
...
class TestPIIMasking:
    ...
    def test_sha256_is_64_chars(self, spark, bronze_orders):
        from notebooks.utils import sha256_col
        df = bronze_orders.filter(F.col("customer_id").isNotNull()) \
            .withColumn("h", sha256_col("customer_id"))
...
class TestGoldAggregations:
    def test_fct_revenue_sums_correctly(self, spark):
        ...
        data = [
            ...
            ("O3", date(2024, 1, 15), "South", 50.00,  "CANCELLED"),  # should be excluded
        ...
        result = (
            ...
            .agg(F.sum("order_amount").alias("total_revenue"), F.count("order_id").alias("total_orders"))
        )
